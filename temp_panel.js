...
const server = http.createServer((req, res) => {
  ...
  // Admin status — relay al bot
  if (url.pathname === '/api/admin-status') {
    try {
      const botReq = http.request({
        hostname: 'localhost', port: 3001, path: '/admin-status', method: 'GET',
      ...
      botReq.on('error', () => {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ paused: [], ignored: [] }));
      });
      botReq.end();
    } catch (e) {
      res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  // Devolver TODOS al bot — relay al bot
  if (url.pathname === '/api/devolver-todos' && req.method === 'POST') {
    const botReq = http.request({
      hostname: 'localhost', port: 3001, path: '/devolver-todos', method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Content-Length': 2 }
    }, botRes => {
      let data = '';
      botRes.on('data', chunk => data += chunk);
    ...
    botReq.on('error', () => {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: 'Bot no disponible' }));
    });
    botReq.write('{}');
    botReq.end();
    return;
  ...
  // Des-ignorar — relay al bot
  if (url.pathname === '/api/des-ignorar' && req.method === 'POST') {
    let bodyData = '';
    req.on('data', chunk => bodyData += chunk);
    req.on('end', () => {
      const botReq = http.request({
        hostname: 'localhost', port: 3001, path: '/des-ignorar', method: 'POST',
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Vista comprobantes */
  #viewClientes, #viewComprobantes, #viewPostventa, #viewAdmin { display: none; }
  #viewClientes.active, #viewPostventa.active, #viewAdmin.active { display: flex; }
...
<div class="main-tabs">
  ...
  <div class="main-tab" id="mainTabAdmin" onclick="switchMainTab('admin')">🛡️ Admin <span class="badge" id="adminBadge" style="display:none">0</span></div>
...
<!-- Vista: Admin (pausados + ignorados) -->
<div id="viewAdmin" class="container">
  <div class="panel" style="flex:1;min-width:400px;">
    <div class="panel-header">
      <span>🛡️ Clientes Pausados (<span id="pausedCount">0</span>)</span>
      <button class="refresh-btn" style="font-size:11px;padding:5px 12px;" onclick="devolverTodos()">🤖 Devolver TODOS al Bot</button>
    </div>
    <div class="panel-body" id="pausedList"><div class="chat-empty">Cargando...</div></div>
  </div>
  <div class="panel" style="flex:1;min-width:400px;">
    <div class="panel-header">
      <span>🔇 Ignorados (<span id="ignoredCount">0</span>)</span>
    </div>
    <div class="panel-body" id="ignoredList"><div class="chat-empty">Cargando...</div></div>
  </div>
</div>
...
async function selectClient(phone) {
  ...
  document.getElementById('clientDetail').innerHTML = \`
    <div class="client-detail">
      ...
      \${client.memory ? '<div class="memory-box">🧠 <strong>Memoria CRM:</strong>n' + client.memory + '</div>' : ''}
...
function switchMainTab(tab) {
  ...
  document.getElementById('mainTabAdmin').classList.toggle('active', tab === 'admin');
  ...
  document.getElementById('viewAdmin').classList.toggle('active', tab === 'admin');
...
// ====== ADMIN — Pausados + Ignorados ======
async function loadAdminData() {
  try {
    const res = await fetch('/api/admin-status');
    const { paused, ignored } = await res.json();
    const total = paused.length + ignored.length;
    ...
    if (paused.length === 0) {
      document.getElementById('pausedList').innerHTML = '<div class="chat-empty">Ningún cliente pausado \u2014 el bot atiende a todos</div>';
    } else {
      document.getElementById('pausedList').innerHTML = paused.map(p => {
        const client = allData ? allData.clients.find(c => c.phone === p.phone) : null;
        const name = (client && client.name) ? client.name : formatPhone(p.phone);
        return '<div class="client-row" style="justify-content:space-between;">' +
          '<div style="display:flex;align-items:center;gap:10px;">' +
          '<div class="client-avatar" style="color:#d29922;">\u23F8</div>' +
          '<div><div class="client-name">' + name + '</div>' +
          '<div class="client-phone">' + p.phone + ' \u2014 ' + p.minutesLeft + ' min restantes</div></div></div>' +
          '<button class="crm-chip" style="background:#0d3b66;border-color:#1f6feb;color:#58a6ff;margin:0;cursor:pointer;" ' +
          'onclick="devolverUno('' + p.phone + '')">🤖 Devolver al Bot</button></div>';
      }).join('');
    }
    document.getElementById('ignoredCount').textContent = ignored.length;
    if (ignored.length === 0) {
      document.getElementById('ignoredList').innerHTML = '<div class="chat-empty">Ningún contacto ignorado</div>';
    } else {
      document.getElementById('ignoredList').innerHTML = ignored.map(c => {
        const name = c.name || formatPhone(c.phone);
        const mem = c.memory ? c.memory.substring(0, 60) : '';
        return '<div class="client-row" style="justify-content:space-between;">' +
          '<div style="display:flex;align-items:center;gap:10px;">' +
          '<div class="client-avatar" style="color:#f85149;">🔇</div>' +
          '<div><div class="client-name">' + name + '</div>' +
          '<div class="client-phone">' + c.phone + '</div>' +
          (mem ? '<div class="client-memory">💭 ' + mem + '</div>' : '') + '</div></div>' +
          '<button class="crm-chip" style="background:#238636;border-color:#3fb950;color:#fff;margin:0;cursor:pointer;" ' +
          'onclick="desIgnorar(\\'' + c.phone + '\\')">🔊 Reactivar</button></div>';
      }).join('');
    }
  } catch(e) { console.error('Error loadAdminData:', e); }
}

async function devolverTodos() {
  if (!confirm('¿Devolver TODOS los clientes pausados al bot?')) return;
  try {
    const res = await fetch('/api/devolver-todos', { method: 'POST' });
    const data = await res.json();
    if (data.ok) { alert('✅ ' + data.total + ' clientes devueltos al bot'); loadAdminData(); }
  } catch(e) { alert('❌ Error conectando con el bot'); }
}

async function devolverUno(phone) {
  try {
    const res = await fetch('/api/devolver-bot', {
...
async function desIgnorar(phone) {
  try {
    const res = await fetch('/api/des-ignorar', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ phone })
    });
    const data = await res.json();
    if (data.ok) { loadAdminData(); await loadData(); }
  } catch(e) { alert('❌ Error conectando'); }
}
</script>
</body>
</html>`;
